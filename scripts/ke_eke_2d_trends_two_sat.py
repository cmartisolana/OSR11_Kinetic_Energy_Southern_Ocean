"""Compute pixel-wise KE/EKE 2D trends from monthly half-power outputs (two_sat product).

This variant points to an alternative monthly half-power-points input
directory that should contain fields derived from the Copernicus
`two_sat` product. It writes a separate NetCDF so figures can be made
and compared with the ``all_sat`` results.
...
# Expect two-sat-derived monthly half-power outputs under this folder.
INPUT_HP_DIR = REPO_ROOT / "outputs" / "monthly_half-power_points_two_sat"
OUTPUT_DIR = REPO_ROOT / "outputs" / "trends"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = OUTPUT_DIR / "ke_eke_2d_trends_two_sat.nc"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compute 2D KE/EKE trends (two_sat product).")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OUTPUT_FILE,
        help="Output NetCDF path (default: outputs/trends/ke_eke_2d_trends_two_sat.nc).",
    )
    parser.add_argument("--overwrite", action="store_true", help="Recompute even if the output file already exists.")
    ...
    print("=" * 70)
    print("COMPUTE 2D KE/EKE TRENDS (TWO_SAT PRODUCT)")
    print("=" * 70)
    print(f"Input monthly fields: {INPUT_HP_DIR}")
    print(f"Output file: {args.output}")

    if args.output.exists() and not args.overwrite:
        print(f"Output already exists at {args.output}; skipping recomputation.")
        return
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            "description": "Pixel-wise KE/EKE means and Theil-Sen trends (two_sat product)",
            "units_mean": "cm2 s-2",
