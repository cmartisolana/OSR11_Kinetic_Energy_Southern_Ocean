"""Compute pixel-wise KE/EKE 2D trends from monthly half-power outputs.

This script separates expensive trend computation from plotting.
It writes a single NetCDF file consumed by plotting notebooks/scripts.

Usage
-----
python scripts/ke_eke_2d_trends.py
python scripts/ke_eke_2d_trends.py --stride 2 --min-points 24
"""
...
OUTPUT_FILE = OUTPUT_DIR / "ke_eke_2d_trends.nc"


def theil_sen_with_ci(x: np.ndarray, y: np.ndarray) -> dict[str, float]:
    """Return Theil-Sen slope/intercept and slope CI bounds."""
...
def _mk_s_var(y: np.ndarray) -> tuple[float, float]:
    """Return Mann-Kendall S statistic and tie-corrected variance."""
    n = len(y)
    s = 0.0
    for i in range(n - 1):
...
def mann_kendall_yue_wang(y: np.ndarray, alpha: float = 0.05, max_lag: int = 12) -> dict[str, float]:
    """Mann-Kendall significance with Yue-Wang autocorrelation correction."""
    y = np.asarray(y, dtype=float)
    y = y[np.isfinite(y)]
    n = len(y)
    if n < 5:
    ...
    # Detrend before autocorrelation estimation to avoid inflating rho from the trend itself.
...
def load_monthly_ke_eke(input_dir: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Load monthly KE/EKE fields and decimal-year coordinates."""
...
def compute_pixelwise_trends(
    ...
    """Compute Theil-Sen slopes and MK p-values per grid cell."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compute 2D KE/EKE trends for plotting.")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OUTPUT_FILE,
        help="Output NetCDF path (default: outputs/trends/ke_eke_2d_trends.nc).",
    )
    ...
    print("=" * 70)
    print("COMPUTE 2D KE/EKE TRENDS")
    print("=" * 70)
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            "description": "Pixel-wise KE/EKE means and Theil-Sen trends with Yue-Wang MK significance",
            "units_mean": "cm2 s-2",
