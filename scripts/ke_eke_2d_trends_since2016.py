"""Compute pixel-wise KE/EKE 2D trends from monthly half-power outputs (since 2016).

This is a lightweight variant of `ke_eke_2d_trends.py` that restricts
the analysis to monthly samples from 2016.0 onwards and writes a
separate NetCDF file so figures can be generated from this subset.

Usage
-----
python scripts/ke_eke_2d_trends_since2016.py
"""
...
OUTPUT_FILE = OUTPUT_DIR / "ke_eke_2d_trends_since2016.nc"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compute 2D KE/EKE trends (since 2016).")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OUTPUT_FILE,
        help="Output NetCDF path (default: outputs/trends/ke_eke_2d_trends_since2016.nc).",
    )
    ...
    print("=" * 70)
    print("COMPUTE 2D KE/EKE TRENDS (SINCE 2016)")
    print("=" * 70)
    ...
    # Restrict to >= 2016.0
    mask_time = time_decimal >= 2016.0
    if mask_time.sum() == 0:
        raise RuntimeError("No monthly samples >= 2016 found in input directory.")
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            "description": "Pixel-wise KE/EKE means and Theil-Sen trends (since 2016)",
            "units_mean": "cm2 s-2",
